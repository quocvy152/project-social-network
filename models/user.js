...
let isPassWithValidChar = (pass) => {
    ...
    if(isNumChar && isUpperChar && isLowerChar && isSpecialChar) 
        return true;
    else 
        return false;
}
...
/**
 * hàm tạo ra chuỗi ngày tháng năm sinh để truyền vào new Date() cho đúng định dạng 
 * Vd: 2 -> 02, 8 -> 08
 * Lưu ý: Month: 0-11
 *  */ 
let validStrBirthday = (birthdayDate, birthdayMonth, birthdayYear) => {
    if(birthdayMonth.length === 1) birthdayMonth = '0' + birthdayMonth; 
    if(birthdayDate.length === 1) birthdayDate = '0' + birthdayDate;
...
let checkAllErrorRegister = async (email, password, confirmPass, username, phone, birthdayDate, birthdayMonth, birthdayYear) => {
    ...
    /**
     * Hàm kiểm tra về tính hợp lệ của mật khẩu
     */
    ...
    /**
     * hàm kiểm tra xem SĐT có tồn tại chưa
     * hàm kiểm tra xem SĐT có đúng định dạng
     */
    else if(isExistPhone) return "Số điện thoại đã được đăng kí";

    else if(isNaN(phone) || phone.length != 10) return "Số điện thoại không hợp lệ";

    else if(birthdayOfUser > currentDay) return "Ngày sinh không hợp lệ";

    else return null;
}

let checkAllErrorLogin = async (email, password) => {
    if(!email || !password) return "Vui lòng không được bỏ trống";

    let isExistEmail = await USER_COLL.findOne({ email });
    if(!isExistEmail) return "Email không tồn tại";
    
    let isMatchPass = await compare(password, isExistEmail.password);
    if(!isMatchPass) return "Mật khẩu không hợp lệ";
